...
<div class="card-header d-flex justify-content-between align-items-center flex-wrap">
    <h4 class="card-title mb-2 mb-sm-0">🎥 QR Attendance OUT System</h4>

    <div class="d-flex gap-2">
        <button id="startButton" class="btn btn-danger">
            <i class="fa fa-play-circle"></i> Start Exit Scan
        </button>
...
<div id="status" class="alert alert-info text-center mb-3">
    Click “Start Exit Scan” to begin scanning.
</div>
...
<div class="border p-3 rounded bg-light">
    <h5><i class="fa fa-list"></i> Exit Scanned List</h5>
    <div id="attendanceList" style="max-height:300px; overflow-y:auto;">
        <p class="text-muted">No exit marked yet...</p>
    </div>
</div>
...
function scanQRCode() {
    ...
    if (code) {
        ...
        if (Date.now() - lastScan > 10000) {
            ...
            saveAttendanceOut(data.id);
        }
    }
    requestAnimationFrame(scanQRCode);
}

function saveAttendanceOut(studentId) {
    fetch('attendance_out_save.php', {
        method: 'POST',
